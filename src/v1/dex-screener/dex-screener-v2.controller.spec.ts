...
describe('DexScreenerV2Controller', () => {
  let controller: DexScreenerV2Controller;
  ...
  beforeEach(async () => {
    ...
    controller = module.get<DexScreenerV2Controller>(DexScreenerV2Controller);
  ...
  describe('events', () => {
    it('should return swap events', async () => {
      const mockEvents = [
        {
          eventType: 'swap',
          blockNumber: 1000,
      ...
      expect(result.events[0].pairId).toBe('123');
      expect(result.events[0].asset0In).toBe('100');
      expect(result.events[0].asset1Out).toBe('200');
    });

    it('should return join events', async () => {
      const mockEvents = [
        {
          eventType: 'join',
          blockNumber: 1000,
  ...
  describe('pair', () => {
    it('should return pair info', async () => {
      ...
      const result = await controller.pair(ExchangeId.OGEthereum, { id: '123' });
