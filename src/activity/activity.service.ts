...
export class ActivityService {
  ...
  async update(endBlock: number, deployment: Deployment): Promise<void> {
    ...
    for (let batchStart = startBlock; batchStart < endBlock; batchStart += BATCH_SIZE) {
      ...
      // Get the query for this batch
      const query = this.getActivityQuery(batchStart, batchEnd, deployment);

      const result = await this.dataSource.query(query);

      // Save results in smaller batches
      const saveBatchSize = 1000;
      for (let i = 0; i < result.length; i += saveBatchSize) {
        const batch = result.slice(i, i + saveBatchSize).map((record) => ({
      ...
      // Update the last processed block number after each batch
