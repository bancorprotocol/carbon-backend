...
export class ActivityService {
  ...
  async update(endBlock: number): Promise<void> {
    ...
    const query = `
          ...
          SELECT
              *,
              ROW_NUMBER() OVER (
                  ...
                  ORDER BY
                      "id" DESC
              ) AS rn -- ordering BY "id" here IS actually the log id
          FROM
      ...
      updated_insert AS (
          SELECT
              1 AS sorting_order,
              -- we are about to insert this prior line infront of the other updates
      ...
      updated AS (
          SELECT
              ...
              -- "id" here IS actually the log id - best for sorting
              timestamp AS evt_block_time,
              "blockId" AS evt_block_number,
              s."strategyId" AS id,
              order0,
  ...
  async getFilteredActivities(params: ActivityDto | ActivityMetaDto): Promise<Activity[]> {
    ...
    if (params.actions) {
      ...
      queryBuilder.andWhere('activity.action IN (:...actions)', { actions });
    }
