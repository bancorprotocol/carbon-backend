...
cmd_test() {
  ...
  if docker exec "$CONTAINER_NAME" test -f /tmp/seed-complete 2>/dev/null; then
    ...
    red "  FAIL: Seed FAILED (check logs: npm run preview:logs)"
    FAIL=$((FAIL + 1))
  else
  ...
  if [ "$FAIL" -gt 0 ]; then
    ...
    echo "  npm run preview:logs    # container logs"
    echo "  npm run preview:db      # psql into container PG"
    echo "  npm run preview:shell   # bash into container"
    exit 1
  else
