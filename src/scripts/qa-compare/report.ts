...
export function writeReport(run: RunOutput, outDir: string): { summaryPath: string; jsonPath: string } {
  ...
  lines.push(
    `- **Chains:** ${run.args.chains.join(', ')} (chain concurrency=${run.args.chainConcurrency}, endpoint concurrency=${run.args.concurrency})`,
  );
  lines.push(
    `- **Tolerances:** block override=${run.args.blockToleranceOverride ?? 'auto'}, price=${run.args.priceTolerancePct}%, volume=${run.args.volumeTolerancePct}%`,
  );
  lines.push(`- **Heavy endpoints (history/simulator):** ${run.args.includeHistorical ? 'on' : 'off'}`);
  lines.push('');
  lines.push('## Summary');
  lines.push('');
  lines.push(`Totals — PASS: **${run.totals.PASS}**, WARN: **${run.totals.WARN}**, FAIL: **${run.totals.FAIL}**, SKIP: **${run.totals.SKIP}**`);
  lines.push('');
  ...
  for (const c of run.chains) {
    ...
    lines.push(
      `| \`${c.chain}\` | ${c.blockDiff ?? 'n/a'} | ${c.blockTolerance} | ${counts.PASS} | ${counts.WARN} | ${counts.FAIL} | ${counts.SKIP} | ${c.ms} |`,
    );
  }
  lines.push('');
  ...
  for (const c of run.chains) {
    ...
    lines.push('|---|---|---|---:|---:|---|---|');
    const sortedProbes = [...c.probes].sort((a, b) => verdictRank(b.verdict) - verdictRank(a.verdict) || a.endpointId.localeCompare(b.endpointId));
    for (const p of sortedProbes) {
      ...
      lines.push(
        `| \`${p.endpointId}\` | ${VERDICT_EMOJI[p.verdict]} | ${p.classifier} | ${stagingCol} | ${prodCol} | ${escMd(p.reason)} | ${fmtMetrics(p.metrics)} |`,
      );
    }
    lines.push('');
...
function appendProbeDetail(lines: string[], p: ProbeResult) {
  lines.push('');
  lines.push(`<details><summary><code>${p.endpointId}</code> — ${p.verdict}: ${escMd(truncate(p.reason, 200))}</summary>`);
  lines.push('');
  lines.push(`- staging: \`${p.pathStaging}\` → ${p.staging.status} in ${p.staging.ms}ms (${formatBytes(p.staging.sizeBytes)})${p.staging.error ? ` err=${p.staging.error}` : ''}`);
  lines.push(`- prod: \`${p.pathProd}\` → ${p.prod.status} in ${p.prod.ms}ms (${formatBytes(p.prod.sizeBytes)})${p.prod.error ? ` err=${p.prod.error}` : ''}`);
  if (p.metrics) {
