 ...
 *
 * Requires:
 *   - Backend running against the Tenderly testnet (npm run gradient:testnet:create -- --run)
...
async function main() {
  ...
  if (!rpcUrl) {
    console.error('ETHEREUM_RPC_ENDPOINT not set. Run gradient:testnet:create first or source .env.tenderly');
    process.exit(1);
  }
