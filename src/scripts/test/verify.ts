 ...
 * that BOTH regular Carbon and gradient data are correctly mixed into the
 * responses.
 *
 * Covers all 39 endpoints across 17 controllers with intelligent value
 * assertions.
 *
 * Modes:
 *   --mode=integration (default) — Assertions match db:seed:test values
 *                                  (USDC/WETH, exact budgets/dates).
 *   --mode=e2e                   — Assertions match Tenderly-harvested data
 *                                  (DAI/USDC, 100-unit budgets).
 *
 * Usage:
 *   npm run test:verify
 *   npm run test:verify -- --mode=e2e
 *   npm run test:verify -- --base-url=http://localhost:3001
 *   (or: npx ts-node src/scripts/test/verify.ts [...])
 */
...
const MODE: 'integration' | 'e2e' =
  (process.argv.find((a) => a.startsWith('--mode='))?.split('=')[1] as any) || 'integration';

let postTradeData: any = null;
if (MODE === 'e2e') {
  const dataPath = path.join(__dirname, 'e2e-data.json');
  try {
    postTradeData = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
  } catch (_) {
    console.log('  Note: e2e-data.json not found, using approximate budget assertions');
  }
}
...
const INTEGRATION_CONFIG: ModeConfig = {
...
const CFG = MODE === 'e2e' ? E2E_CONFIG : INTEGRATION_CONFIG;
...
async function testStrategies() {
  ...
  try {
    ...
    assert(
      G,
      ep,
      gradients.length === expectedCount,
      `${expectedCount} gradient strategies found`,
      `Expected ${expectedCount} gradient strategies, got ${gradients.length}`,
    );
    ...
    assert(
      G,
      ep,
      types.size === expectedCount,
      `All ${expectedCount} gradient types covered (unique strategies)`,
      `Only ${types.size} unique gradient strategies`,
    );

    if (!CFG.exactDates) {
      const gradientTokenPairs = new Set(
        gradients.map((s: any) => {
    ...
    if (gradients.length > 0) {
      ...
      assert(
        ...
        ep,
        first.sell &&
          typeof first.sell.startPrice === 'string' &&
          typeof first.sell.endPrice === 'string' &&
          typeof first.sell.startDate === 'string' &&
          typeof first.sell.endDate === 'string' &&
          typeof first.sell.budget === 'string' &&
          typeof first.sell.marginalPrice === 'string',
      ...
      if (CFG.exactDates) {
        assert(
          G,
          ep,
          first.sell?.budget === CFG.sellBudget,
        ...
        assert(
          G,
          ep,
          first.buy?.budget === CFG.buyBudget,
      ...
      assert(
        G,
        ep,
        parseFloat(first.sell?.startPrice) > 0,
      ...
      if (CFG.exactDates) {
        assert(
          G,
          ep,
          first.sell?.startDate === CFG.sellStartDate,
        ...
        assert(
          G,
          ep,
          first.sell?.endDate === CFG.sellEndDate,
      ...
      const linearIncrease = gradients.find(
        (s: any) => parseFloat(s.sell?.marginalPrice) > parseFloat(s.sell?.startPrice),
      );
      if (linearIncrease) {
...
async function testSeedData() {
  ...
  try {
    ...
    assert(
      ...
      'schemeVersion is 8',
      `Expected schemeVersion 8, got ${data.schemeVersion}`,
    );
    ...
    assert(
      G,
      ep,
      gradients.length >= expectedSeedCount,
    ...
    if (gradients.length > 0) {
      ...
      if (CFG.exactDates) {
        assert(
          G,
          ep,
          first.order0?.liquidity === CFG.seedOrder0Liquidity,
      ...
      if (CFG.exactDates) {
        assert(
          G,
          ep,
          first.order0?.tradingStartTime === 1730329400,
        ...
        assert(
          G,
          ep,
          first.order0?.tradingStartTime > 0,
...
async function testAnalyticsGeneric() {
  ...
  try {
    ...
    if (metrics.number_trades !== undefined) {
      assert(G, ep, metrics.number_trades > 0, `number_trades=${metrics.number_trades}`, `number_trades is zero`);
    }
  } catch (e: any) {
    fail(G, ep, 'Request failed', e.message);
  }
...
async function testWalletPairBalance() {
  ...
  try {
    ...
    assert(
      G,
      ep,
      pairData && typeof pairData === 'object' && Object.keys(pairData).length > 0,
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      if (gradientActivities.length > 0) {
        const noUnknownTokens = gradientActivities.every(
          (a: any) => a.strategy.base && a.strategy.base !== '' && a.strategy.quote && a.strategy.quote !== '',
        );
        assert(
          G,
          ep,
        ...
        assert(
          ...
          `Some gradient activities have blockNumber 0: ${
            gradientActivities.filter((a: any) => !a.blockNumber).length
          }`,
        );
      }
    }
...
async function testActivityMeta() {
  ...
  try {
    ...
    assert(G, ep, data && typeof data === 'object', 'Returns object', 'Invalid response shape');
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      assert(
        G,
        ep,
        data[0].strategy && typeof data[0].strategy.type === 'string',
...
async function testActivityV2Meta() {
  ...
  try {
    ...
    assert(
      G,
      ep,
      data && data.size !== undefined && Array.isArray(data.actions),
...
async function testTokensPrices() {
  ...
  try {
    ...
    if (ethKey) {
      ...
      assert(G, ep, ethPrice > 1000, `ETH price = ${ethPrice} (> 1000)`, `ETH price too low: ${ethPrice}`);
    } else {
...
async function testCmcPairs() {
  ...
  try {
    ...
    assert(G, ep, Array.isArray(data) && data.length > 0, `${data.length} CMC pairs`, 'No CMC pairs returned');
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(
        ...
        ep,
        'base_id' in first &&
          'base_symbol' in first &&
          'quote_id' in first &&
          'quote_symbol' in first &&
          'pair' in first,
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      if (first.pair) {
        assert(
          G,
          ep,
          first.pair.fromToken &&
            'decimals' in first.pair.fromToken &&
            'symbol' in first.pair.fromToken &&
            'address' in first.pair.fromToken,
...
async function testAnalyticsTvl() {
  ...
  try {
    ...
    assert(G, ep, Array.isArray(data) && data.length > 0, `${data.length} TVL entries`, 'No TVL data returned');
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(
        G,
        ep,
        'timestamp' in first && 'tvlUsd' in first,
...
async function testAnalyticsTvlTokens() {
  ...
  try {
    ...
    assert(G, ep, Array.isArray(data), `Returns array (${data.length} entries)`, 'Expected array response');
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(
        G,
        ep,
        'address' in first || 'symbol' in first || 'tvlUsd' in first,
...
async function testAnalyticsVolume() {
  ...
  try {
    ...
    assert(G, ep, Array.isArray(data) && data.length > 0, `${data.length} volume entries`, 'No volume data returned');
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(
        G,
        ep,
        'timestamp' in first && 'volumeUsd' in first && 'feesUsd' in first,
        'Volume entry has { timestamp, volumeUsd, feesUsd }',
        `Missing fields: ${JSON.stringify(Object.keys(first))}`,
      );

      const volVal = parseFloat(first.volumeUsd);
      assert(G, ep, !isNaN(volVal), `volumeUsd is numeric: ${volVal}`, `volumeUsd is not numeric: ${first.volumeUsd}`);
...
async function testAnalyticsTrending() {
  ...
  try {
    ...
    assert(
      G,
      ep,
      'totalTradeCount' in data && data.totalTradeCount > 0,
    ...
    assert(
      G,
      ep,
      Array.isArray(data.tradeCount) && Array.isArray(data.pairCount),
    ...
    if (Array.isArray(data.tradeCount) && data.tradeCount.length > 0) {
      ...
      const gradPair = data.tradeCount.find((tc: any) => {
        ...
        return (
          (t0 === CFG.gradientToken0.toLowerCase() && t1 === CFG.gradientToken1.toLowerCase()) ||
...
async function testCoingeckoTickers() {
  ...
  try {
    ...
    assert(G, ep, Array.isArray(data) && data.length > 0, `${data.length} tickers returned`, 'No tickers returned');
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(
        ...
        ep,
        'ticker_id' in first &&
          'base_currency' in first &&
          'last_price' in first &&
          'base_volume' in first &&
          'liquidity_in_usd' in first,
...
async function testCoingeckoHistoricalTrades() {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(G, ep, parseFloat(first.price) > 0, `price > 0: ${first.price}`, `price not > 0: ${first.price}`);
    }
  } catch (e: any) {
    fail(G, ep, 'Request failed', e.message);
  }
...
async function testDexScreenerLatestBlock() {
  ...
  try {
    ...
    assert(
      G,
      ep,
      data && data.block && data.block.blockNumber > 0,
      `blockNumber = ${data.block?.blockNumber}`,
      `blockNumber missing or zero: ${JSON.stringify(data)}`,
    );

    assert(G, ep, data.block?.blockTimestamp !== undefined, `blockTimestamp present`, 'blockTimestamp missing');
  } catch (e: any) {
    fail(G, ep, 'Request failed', e.message);
  }
}
...
async function testDexScreenerAsset() {
  ...
  try {
    ...
    assert(G, ep, data && data.asset, 'Returns { asset }', `Invalid shape: ${JSON.stringify(data)}`);

    if (data?.asset) {
      assert(G, ep, data.asset.symbol === 'USDC', `symbol = "USDC"`, `Expected "USDC", got "${data.asset.symbol}"`);

      assert(G, ep, data.asset.decimals === 6, `decimals = 6`, `Expected 6, got ${data.asset.decimals}`);
    }
  } catch (e: any) {
    fail(G, ep, 'Request failed', e.message);
  }
    ...
    try {
      ...
      if (data?.events?.length > 0) {
        ...
        assert(
          G,
          ep,
          first.block && first.block.blockNumber > 0,
    ...
    try {
      ...
      if (data?.events) {
        ...
        assert(
          G,
          ep,
          eventTypes.has('join') || eventTypes.has('swap'),
  ...
  try {
    ...
    if (data?.asset) {
      assert(
        G,
        ep,
        data.asset.symbol === 'USDC' && data.asset.decimals === 6,
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      assert(
        G,
        ep,
        typeof first.ROI === 'number' && typeof first.id === 'string',
...
async function testState() {
  ...
  try {
    ...
    assert(G, ep, data && data.lastBlock > 0, `lastBlock = ${data.lastBlock}`, `lastBlock missing or zero`);

    assert(
      G,
      ep,
      data.timestamp !== undefined && !isNaN(new Date(data.timestamp).getTime()),
...
async function testHistoryPrices() {
  ...
  try {
    ...
    assert(G, ep, Array.isArray(data), `Returns array (${data.length} candles)`, 'Expected array response');
...
async function main() {
  console.log(`\nAPI Verification (Carbon + Gradient) — ${MODE.toUpperCase()} mode`);
  ...
  const groups: { name: string; tests: (() => Promise<void>)[] }[] = [
    ...
    },
    {
      ...
      tests: [
        testAnalyticsTvl,
        testAnalyticsTvlTokens,
        testAnalyticsVolume,
        testAnalyticsVolumeTokens,
        testAnalyticsTrending,
      ],
    },
    {
      name: 'Group E — Coingecko/DexScreener/GeckoTerminal',
      tests: [
        testCoingeckoTickers,
        testCoingeckoHistoricalTrades,
        testDexScreenerLatestBlock,
        testDexScreenerAsset,
        testDexScreenerEvents,
        testGeckoTerminalLatestBlock,
        testGeckoTerminalAsset,
        testGeckoTerminalEvents,
      ],
    },
    {
    ...
    },
    {
      ...
      tests: [
        testAnalyticsTvlPairs,
        testAnalyticsVolumePairs,
        testMerkleData,
        testMerkleRewards,
        testGeckoTerminalPair,
        testDexScreenerPair,
      ],
    },
  ];
