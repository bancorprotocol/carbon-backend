...
import { GradientStrategyDeletedEvent } from './gradient-strategy-deleted-event.entity';
...
@Injectable()
export class GradientStrategyDeletedEventService {
  private readonly logger = new Logger(GradientStrategyDeletedEventService.name);

  constructor(
    @InjectRepository(GradientStrategyDeletedEvent)
    private repository: Repository<GradientStrategyDeletedEvent>,
  ...
  async update(endBlock: number, deployment: Deployment, tokens: TokensByAddress, pairsDictionary: PairsDictionary): Promise<void> {
    ...
    const key = `${deployment.blockchainType}-${deployment.exchangeId}-gradient-strategy-deleted-events`;
    ...
    if (events.length > 0) {
      this.logger.log(`[Gradient] Harvested ${events.length} StrategyDeleted events for ${deployment.exchangeId}`);
      ...
      const entities = events.map((e) => {
        ...
        if (!token0 || !token1) {
          throw new Error(`[Gradient] Token not found for StrategyDeleted event: token0=${t0Addr} (found=${!!token0}), token1=${t1Addr} (found=${!!token1})`);
        }
  ...
  async get(startBlock: number, endBlock: number, deployment: Deployment): Promise<GradientStrategyDeletedEvent[]> {
