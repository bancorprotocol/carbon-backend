...
export class UpdaterService {
  ...
  private async initWssRealtimeStrategy(deployment: Deployment): Promise<void> {
    ...
    try {
      // Initial full sync to establish baseline (regular + gradient)
      ...
      // Start WSS event listeners (regular + gradient)
      await this.strategyRealtimeService.startEventListener(deployment, tokens);
      if (this.deploymentService.hasGradientSupport(deployment)) {
        await this.gradientRealtimeService.startEventListener(deployment, tokens);
      }

      // Schedule guarded full sync every 60s as safety net (covers both)
      setInterval(async () => {
  ...
  private async updateRealtimeStrategiesGuarded(deployment: Deployment): Promise<void> {
    ...
    try {
      ...
      if (this.deploymentService.hasGradientSupport(deployment)) {
        this.gradientRealtimeService.updateTokens(deployment, tokens);
        await this.gradientRealtimeService.update(deployment, tokens, true);
      }
