...
```bash
npm run rewards:generate-strategy-csv -- --deployment <name> --output strategy_events.csv
```

**Options:**

- `--deployment <name>` — Deployment to export. Available deployment names are defined in `DEPLOYMENT_MAP` inside `src/scripts/rewards/generate-strategy-csv.ts`.
